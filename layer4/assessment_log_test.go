...
func TestRunStep(t *testing.T) {
	...
	for _, test := range stepsTestData {
		t.Run(test.testName, func(t *testing.T) {
			...
			if result.Status != test.result {
				t.Errorf("expected %s, got %s", test.result, result)
			}
			if anyOldAssessment.Result.Status != test.result {
...
func TestRun(t *testing.T) {
	for _, data := range getAssessmentsTestData() {
		t.Run(data.testName, func(t *testing.T) {
			...
			if result != a.Result.Status {
				t.Errorf("expected match between Run return value (%s) and assessment Status value (%s)", result, data.expectedResult)
			}
