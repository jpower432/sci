...
Start at Layers 1 & 2. Those are required imports into Layer 3 artifacts (Policy). Layers 5-7 (Evaluation, Enforcement, Audit) require at least Layer 2.

You do not need artifacts at every layer to get value. A Control Catalog (Layer 2) is immediately useful on its own.

### Which layer do I start with?

Layers are separated by **persona** and **job to be done**, not sequential steps:

| **Layer** | **Who** | **Job** |
|:--|:--|:--|
| 1 | Standards bodies, governance teams | Publish generic guidance and best practices |
...
| 5-7 | Tools and auditors | Evaluate, enforce, and audit compliance |

If you're a security engineer, you're mostly living in Layer 2 and *consuming* Layer 1 artifacts that someone else published (NIST, CIS, an internal architecture team).
...
```
  ...
  ┌─────────────────────┐     ┌─────────────────────┐
  │   NIST SSDF          │     │   MITRE ATT&CK      │
  │   Guidance Catalog    │     │   Vector Catalog     │
  │   "How software       │     │   "How attackers     │
  │    should be made"    │     │    target supply      │
  │                       │     │    chains"            │
  └──────────┬────────────┘     └──────────┬────────────┘
             │ informs                      │ maps to
             ▼                              ▼
  ...
  ┌─────────────────────┐     ┌─────────────────────┐
  │   OSPS Baseline      │     │   Threat Catalog     │
  │   Control Catalog     │     │   "What can go wrong │
  │   "Testable controls  │◄────│    for this project" │
  │    for OS projects"   │     │                      │
  └──────────┬────────────┘     └──────────────────────┘
             │ imported by
             ▼
  ...
  │   "Our projects must comply with OSPS Baseline"   │
  │   Scope: all public repos | Frequency: weekly     │
  └──────────┬───────────────────────────────────────┘
             │ applied to
             ▼
  ...
  │   (existing tooling — Gemara doesn't schema this) │
  └──────────┬───────────────────────────────────────┘
             │ evaluated by
             ▼
  Layer 5 (Evaluation)
  ┌──────────────────────────────────────────────────┐
  │   Evaluation Log                                  │
  │   Scanner checks repo against policy controls     │
  │   Result: branch protection missing ✗             │
  └──────────┬───────────────────────────────────────┘
             │ triggers
             ▼
  Layer 6 (Enforcement)
  ┌──────────────────────────────────────────────────┐
  │   Enforcement Log                                 │
  │   Tooling enables branch protection automatically │
  │   Action: remediated ✓                            │
  └──────────┬───────────────────────────────────────┘
             │ reviewed by
             ▼
  ...
  ┌──────────────────────────────────────────────────┐
  │   Audit Log                                       │
  │   Auditor reviews: policy existed, evaluation     │
  │   found gap, enforcement remediated, passing since│
  └──────────────────────────────────────────────────┘
```

**Who does what in this scenario:**

- **Standards bodies** (NIST, MITRE) publish Layer 1 artifacts — you consume them
- **The OSPS project** publishes the Layer 2 Control Catalog — you consume it
- **Your governance body** (OpenSSF TAC, your company's security team) writes the Layer 3 Policy importing OSPS Baseline
...
**Guidance** is generic — it applies across technologies. **Controls** are specific, actionable, and assessable for a particular technology.
...
Mapping Documents are a separate artifact where **anyone** (author or consumer) can map atomic units between catalogs with more fidelity. Consumers may disagree with a vendor's mappings or want to add mappings without modifying the original catalog. Mapping Documents enable that without forking the source.

### What is Layer 4 and why is there no schema?

Layer 4 is symbolic. It represents the actual deployment and operation of systems — covered by existing ecosystems (Kubernetes, Terraform, CI/CD pipelines, SBOM generators).
...
No. OSCAL is mature with broad adoption in regulated environments.
Organizations producing or consuming OSCAL artifacts should continue doing so. 
Gemara addresses a different need — the engine that *produces* compliance outcomes, not the format that *reports* them. Organizations benefit from using both.

## Adopting Gemara

### Do I have to start from scratch?

No. Each schema is valuable on its own and you can adopt incrementally. If you have controls in a spreadsheet, that's a great first transformation into a Control Catalog. Threat models can come next. Your existing tooling (OPA policies, Terraform modules, scanners) stays as-is — Gemara describes the relationships between them, it doesn't replace them.
...
AI tooling can help. The Gemara MCP Server assists agents with migrations — feed it your existing policy, and it can help structure it into the schema. Otherwise, convert one at a time. Start with your engineering standards.
...
### How does evidence collection work across multiple frameworks?

Write the policy once, map it to multiple frameworks. If your access control policy satisfies both SOC 2 CC6.1 and ISO 27001 A.9, you write one policy, one set of controls, and one set of evaluations. The mapping handles the rest. Engineering produces evidence once; compliance consumes it for every framework it satisfies.
...
Your existing tools (OPA, scanners, CI checks) stay as-is. You may need glue code to ensure they produce Evaluation Logs in the Gemara schema, or you can use tools in the Gemara ecosystem that already integrate. The key: each evaluation result references an assessment requirement ID from the imported Control Catalog, so it traces back through the chain — threat → control → assessment requirement → evaluation → enforcement.
