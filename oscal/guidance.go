...
func FromGuidance(g *gemara.GuidanceDocument, guidanceDocHref string, opts ...GenerateOption) (oscal.Catalog, oscal.Profile, error) {
	...
	alterationMap := processExtendsMappings(g.Guidelines, importMap, options.imports, g.Metadata.Id)
...
// guidelineToStatements converts a guideline to OSCAL parts that can be added to an existing control.
...
func guidelineToStatements(guideline gemara.Guideline, controlId string, guidelineId string) []oscal.Part {
...
func guidelineToControl(g *gemara.GuidanceDocument, guideline gemara.Guideline, resourcesMap map[string]string) (oscal.Control, string) {
	...
	parts := guidelineToStatements(guideline, controlId, "")
	control.Parts = ensureRequiredParts(parts, controlId)

	parentId := ""
	if guideline.Extends != nil {
...
func processAlteration(alterationMap map[string]*oscal.Alteration, normalizedId string, guideline gemara.Guideline, frameworkPrefix string) {
	...
	parts := guidelineToStatements(guideline, normalizedId, frameworkPrefix)
	if len(parts) == 0 {
		return
	}
...
// processExtendsMappings processes guidelines that extend external controls, updating
// importMap with necessary control selectors and returning a map of alterations.
func processExtendsMappings(guidelines []gemara.Guideline, importMap map[string]oscal.Import, imports map[string]string, frameworkPrefix string) map[string]*oscal.Alteration {
...
// ensureRequiredParts ensures that statement and assessment-objective parts always exist
// as required by OSCAL, creating empty defaults if needed.
func ensureRequiredParts(parts []oscal.Part, controlId string) *[]oscal.Part {
