	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err == nil) == tt.wantErr {
				t.Errorf("Guidance.LoadFile() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !tt.wantErr {
				...
				if len(g.Guidelines) == 0 {
					t.Errorf("GuidanceDocument.LoadFile() did not load any guidelines")
				}
			}
		})
	}
...
func Test_LoadFiles_AppendsData(t *testing.T) {
	...
	require.Greater(t, len(one.Guidelines), 0, "expected at least one guideline in good-aigf.yaml")
	...
	assert.Equal(t, one.Title, g.Title, "first document's title should be preserved")
	assert.Equal(t, len(one.Guidelines)*2, len(g.Guidelines), "guidelines should be appended across multiple files")
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil && tt.wantErr {
				...
				t.Errorf("Guidance.LoadFile() expected error matching %s, got nil.", tt.errorExpected)
			} else if err != nil && !tt.wantErr {
				t.Errorf("Guidance.LoadFile() did not expect error, but got '%s'", err.Error())
			}
		})
	}
}
